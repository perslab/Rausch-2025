...
dread_data %>% filter(Exp == "Ghrelin") %>%
  pivot_longer(starts_with("FEN")) %>%
  mutate(value = log1p(value)) %>%
  lme4::lmer(value ~ Trt*(Time) + (1|name), data=.) %>%
  car::Anova(.)
  emmeans::emmeans(., pairwise~Trt|Time)

dread_data %>% filter(Exp == "DarkCycle") %>%
  pivot_longer(starts_with("FEN")) %>%
  mutate(value = log1p(value)) %>%
  lme4::lmer(value ~ Trt*(Time) + (1|name), data=.) %>%
  car::Anova(.)
...
data <- read.csv("glp1r_leptin/2(A)_DIO:Chow_Glp1RCre; LepRFlox Body composition BW - 09 26 2024 update.csv") %>% 
  mutate(X = c("Female_Chow","Male_Chow","Male_HFD")) %>% 
  pivot_longer(-X) %>% 
  mutate(name = gsub("[.][0-9]","", name)) %>% 
  separate(X, into=c("sex","diet"), sep="_") 
```
